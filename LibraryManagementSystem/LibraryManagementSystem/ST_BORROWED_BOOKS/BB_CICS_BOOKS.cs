...
{
    public partial class BB_CICS_BOOKS : Form
    {
        ...
        // Constructor for BB_CICS_BOOKS
        public BB_CICS_BOOKS()
        {
            InitializeComponent();
        ...
        private void BB_CICS_BOOKS_Load(object sender, EventArgs e)
        {
        ...
        // Method to get a list of borrowed books
        ...
        private void BorrowBookFromLibrary(string srcode, string bookId, string bookName)
        {
            try
            {
                ...
                string checkAvailabilityQuery = "SELECT Quantity FROM librarymanagementsystem.cics_book WHERE Book_ID = @bookId";
                using (MySqlCommand checkAvailabilityCommand = new MySqlCommand(checkAvailabilityQuery, fromDatabase))
                {
                    ...
                    {
                        // Update the quantity of the book in the cics_book table
                        string updateQuantityQuery = "UPDATE librarymanagementsystem.cics_book SET Quantity = Quantity - 1 WHERE Book_ID = @bookId";
                        using (MySqlCommand updateQuantityCommand = new MySqlCommand(updateQuantityQuery, fromDatabase))
                        {
                            ...
                            if (rowsAffected > 0)
                            {
                                ...
                                string insertBorrowedBookQuery = "INSERT INTO librarymanagementsystem.cics_borrowed_books (Student_ID, Book_ID, Book_Name, Borrowed_Date) " +
        ...
        private int GetNumberOfBorrowedBooks(string studentId)
        {
            ...
            string query = "SELECT COUNT(*) FROM librarymanagementsystem.cics_borrowed_books WHERE Student_ID = @studentId";
        ...
        // Event handler for navigating to the book list form
        private void button1_Click(object sender, EventArgs e)
        {
            BB_BOOKS_LIST BB_CICS_BOOKS = new BB_BOOKS_LIST();
            BB_CICS_BOOKS.Show();
            this.Close();
        }
    }
