...
{
    public partial class BB_CTE_BOOKS : Form
    {
        ...
        private void BB_CTE_BOOKS_Load(object sender, EventArgs e)
        {
        ...
        private void LoadDataGridView()
        {
            ...
            {
                fromDatabase.Open();
                string selectQuery = "SELECT * FROM librarymanagementsystem.cte_borrowed_books";
        ...
        private void searchData(string valueToSearch)
        {
            ...
            {
                fromDatabase.Open();
                string query = "SELECT * FROM librarymanagementsystem.cte_borrowed_books WHERE  CONCAT(Student_ID, Book_ID, Borrowed_Date) like '%" + valueToSearch + "%'";
        ...
        private void addBook_Click(object sender, EventArgs e)
        {
            ...
            {
                // Borrow the book
                BorrowBookFromLibrary(src, bookId, bookName);
            }
            else
            {
        ...
        private void BorrowBookFromLibrary(string src, string bookId, string bookName)
        {
            try
            {
                fromDatabase.Open();
                ...
                string checkAvailabilityQuery = "SELECT Quantity FROM librarymanagementsystem.cte_book WHERE Book_ID = @bookId";
                using (MySqlCommand checkAvailabilityCommand = new MySqlCommand(checkAvailabilityQuery, fromDatabase))
                {
                    ...
                    {
                        // Update the quantity of the book in the cit_book table
                        string updateQuantityQuery = "UPDATE librarymanagementsystem.cte_book SET Quantity = Quantity - 1 WHERE Book_ID = @bookId";
                        using (MySqlCommand updateQuantityCommand = new MySqlCommand(updateQuantityQuery, fromDatabase))
                        {
                            ...
                            if (rowsAffected > 0)
                            {
                                ...
                                string insertBorrowedBookQuery = "INSERT INTO librarymanagementsystem.cte_borrowed_books (Student_ID, Book_ID, Book_Name, Borrowed_Date) " +
                                ...
                                {
                                    insertBorrowedBookCommand.Parameters.AddWithValue("studentId", src);
                                    ...
                                    if (borrowedRowsAffected > 0)
                                    {
                                        ...
                                        int numberOfBorrowedBooks = GetNumberOfBorrowedBooks(src);

                                        if (numberOfBorrowedBooks >= 3)
                                        {
                                            // Retrieve the student's name
                                            string studentName = GetStudentName(src);

                                            // Implement your reward logic here
                                            MessageBox.Show($"Congratulations, {studentName}!  Your commitment to reading is truly inspiring! You've borrowed books consistently, and that's a fantastic achievement. As a token of appreciation, we're giving you a free stationary paper and a ballpen. Please claim it to the main desk of the library, Thank you.", "Reward", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                        }
                                    }
                                    else
                                    {
        ...
        // Method to get the number of borrowed books by a student
        private int GetNumberOfBorrowedBooks(string studentId)
        {
            // Query to get the number of books borrowed by a student
            string query = "SELECT COUNT(*) FROM librarymanagementsystem.cte_borrowed_books WHERE Student_ID = @studentId";
        ...
        private string GetStudentName(string src)
        {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            BB_BOOKS_LIST BB_CTE_BOOKS = new BB_BOOKS_LIST();
            BB_CTE_BOOKS.Show();
            this.Close();
        }
    }
