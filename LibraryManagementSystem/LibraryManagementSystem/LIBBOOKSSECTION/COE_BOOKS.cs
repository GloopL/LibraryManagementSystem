...
{
    public partial class COE_BOOKS : Form
    {
        ...
        private void COE_BOOKS_Load(object sender, EventArgs e)
        {
        ...
        private void AddNewBook(string bookId, string bookName, string author, string publisher, string quantity)
        {
            ...
            try
            {
                ...
                string insertQuery = "INSERT INTO librarymanagementsystem.coe_book (`Book_ID`,`Book_Name`, `Author`, `Publisher`, `Quantity`) " +
        ...
        private bool EditBook(string bookId, string bookName, string author, string publisher, string quantity)
        {
            try
            {
                using (fromDatabase = new MySqlConnection("datasource=localhost;port=3306;username=root;password=''; database=librarymanagementsystem"))
                {
                    ...
                    string updateQuery = "UPDATE `coe_book` SET `Book_Name`= @bookName, `Author`= @author, `Publisher`= @publisher, `Quantity`= @quantity WHERE `Book_ID`= @bookId";
        ...
        {
            string query = "DELETE FROM `coe_book` WHERE `Book_ID`= @bookId";
        ...
        private void LoadDataGridView()
        {
            ...
            {
                fromDatabase.Open();
                string selectQuery = "SELECT * FROM librarymanagementsystem.coe_book";
        ...
        {
            string query = "SELECT * FROM librarymanagementsystem.coe_book";
        ...
        private void searchData(string valueToSearch)
        {
            ...
            {
                fromDatabase.Open();
                string query = "SELECT * FROM librarymanagementsystem.coe_book WHERE  CONCAT(`Book_ID`,`Book_Name`, `Author`, `Publisher`, `Quantity`) like '%" + valueToSearch + "%'";
        ...
        {
            string selectQuery = $"SELECT * FROM coe_book WHERE Book_Name LIKE '%{searchQuery}%'";
            return db.GetData(selectQuery, null);
        }

        private void button1_Click(object sender, EventArgs e)
        {
            LIBBOOK COE_BOOKS = new LIBBOOK();
            COE_BOOKS.Show();
            this.Close();
        }
