...
{
    public partial class ST_CTE_RETURNED : Form
    {
        ...
        // Constructor for ST_CTE_RETURNED
        public ST_CTE_RETURNED()
        {
            InitializeComponent();
        ...
        private void ST_CTE_RETURNED_Load(object sender, EventArgs e)
        {
        ...
        private void ReturnBook(string studentId, string bookId, string bookName)
        {
            try
            {
                ...
                string checkBorrowedQuery = "SELECT * FROM librarymanagementsystem.cte_borrowed_books WHERE Student_ID = @studentId AND Book_ID = @bookId";
                using (MySqlCommand checkBorrowedCommand = new MySqlCommand(checkBorrowedQuery, fromDatabase))
                {
                    ...
                    using (MySqlDataReader reader = checkBorrowedCommand.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            ...
                            string deleteBorrowedBookQuery = "DELETE FROM librarymanagementsystem.cte_borrowed_books WHERE Student_ID = @studentId AND Book_ID = @bookId";
                            using (MySqlCommand deleteBorrowedBookCommand = new MySqlCommand(deleteBorrowedBookQuery, fromDatabase))
                            {
                                ...
                                if (rowsAffected > 0)
                                {
                                    // Increase the quantity of the returned book in the cte_book table
        ...
        {
            ST_RETURNED_BOOKS_LIST ST_CTE_RETURNED = new ST_RETURNED_BOOKS_LIST();
            ST_CTE_RETURNED.Show();
            this.Close();
        }
    }
}
