...
{
    public partial class ST_CICS_RETURNED : Form
    {
        ...
        private void ST_CICS_RETURNED_Load(object sender, EventArgs e)
        {
        ...
        private void ReturnBook(string studentId, string bookId, string bookName)
        {
            try
            {
                ...
                string checkBorrowedQuery = "SELECT * FROM librarymanagementsystem.cics_borrowed_books WHERE Student_ID = @studentId AND Book_ID = @bookId";
                using (MySqlCommand checkBorrowedCommand = new MySqlCommand(checkBorrowedQuery, fromDatabase))
                {
                    ...
                    using (MySqlDataReader reader = checkBorrowedCommand.ExecuteReader())
                    {
                        if (reader != null && reader.Read())
                        {
                            ...
                            string deleteBorrowedBookQuery = "DELETE FROM librarymanagementsystem.cics_borrowed_books WHERE Student_ID = @studentId AND Book_ID = @bookId";
        ...
        {
            ST_RETURNED_BOOKS_LIST ST_CICS_RETURNED = new ST_RETURNED_BOOKS_LIST();
            ST_CICS_RETURNED.Show();
            this.Close();
        }
    }
}
