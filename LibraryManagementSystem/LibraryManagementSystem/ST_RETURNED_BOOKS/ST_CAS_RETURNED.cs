...
{
    public partial class ST_CAS_RETURNED : Form
    {
        ...
        // Constructor for ST_CAS_RETURNED
        public ST_CAS_RETURNED()
        {
            InitializeComponent();
        ...
        private void ST_CAS_RETURNED_Load(object sender, EventArgs e)
        {
        ...
        private void ReturnBook(string studentId, string bookId, string bookName)
        {
            try
            {
                ...
                string checkBorrowedQuery = "SELECT * FROM librarymanagementsystem.cas_borrowed_books WHERE Student_ID = @studentId AND Book_ID = @bookId";
                using (MySqlCommand checkBorrowedCommand = new MySqlCommand(checkBorrowedQuery, fromDatabase))
                {
                    ...
                    using (MySqlDataReader reader = checkBorrowedCommand.ExecuteReader())
                    {
                        if (reader != null && reader.Read())
                        {
                            ...
                            string deleteBorrowedBookQuery = "DELETE FROM librarymanagementsystem.cas_borrowed_books WHERE Student_ID = @studentId AND Book_ID = @bookId";
                            using (MySqlCommand deleteBorrowedBookCommand = new MySqlCommand(deleteBorrowedBookQuery, fromDatabase))
                            {
                                ...
                                if (rowsAffected > 0)
                                {
                                    // Increase the quantity of the returned book in the cas_book table
        ...
        {
            ST_RETURNED_BOOKS_LIST ST_CAS_RETURNED = new ST_RETURNED_BOOKS_LIST();
            ST_CAS_RETURNED.Show();
            this.Close();
        }
    }
}
